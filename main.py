...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add (output, output1)
    ...
    output2 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same', kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add (output, output2)
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epochs in range(epochs):
        ...
        print ("Epoch number {} is underway and loss is {}".format(epochs, loss))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        #Defining all the placeholders and needed variables
        epochs = 50
        batch_size = 5
